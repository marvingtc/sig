...
class NutritionsControllerInfmeta extends NutritionsController
{
    ...
    function saveAction()
    {
        ...
        $link = 'index.php?option=com_nutritions&controller=infmeta&view=infmeta&task=edit&Itemid=3&&cid[]='.$resultStore;
        $this->setRedirect($link, $msg);
     }
     
    public function removeInfmeta() {
        $infmetaId = JRequest::getInt('infmetaId', NULL);
        $model = $this->getModel('infmeta');
        $model->removeInfmeta($infmetaId);
        $msg = 'Informe Removido!!!';
        $link = 'index.php?option=com_nutritions&view=infmetas';
        $this->setRedirect($link, $msg);
    }
