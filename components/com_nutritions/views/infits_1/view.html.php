...
class NutritionsViewInfits extends JView {
    
    function display($tpl = null) {
        global $mainframe, $option;
        
        $model = $this->getModel();
        $infits = & $this->get('Data');
        $results = array();
       ...
       //$document->addStyleSheet(JURI::base().'components/com_nutritions/assets/css/bootstrap.min.css');
        ...
        $lists['resultado_anho'] = JHTML::_('select.genericlist', $anhoArray, 'nu_ano', 'class="inputbox" size="1"', 'value', 'text', $infits->nu_ano);
        ...
        $lists['resultado_mes'] = JHTML::_('select.genericlist', $mesArray, 'nu_mes', 'class="inputbox" size="1" onblur="validainfrepetido();" ', 'value', 'text', $infits->nu_mes);

        $this->assignRef('lists', $lists);
        $this->assignRef('infits', $infits);
