...
class NutritionsViewInfmeta extends JView {
    
    function display($tpl = null) {
        global $mainframe, $option;
        
        $model = $this->getModel();
        $infmeta = & $this->get('Data');
        $results = array();
        ...
        $document->addScript(JURI::base().'components/com_nutritions/assets/js/jquery.js');
...
//        $anhos = $model->getDetalleGeneral('33', '211');
        ...
        $anhoArray[] = JHTML::_('select.option', '255', JText::_('2015'));
        foreach ($anhos as $anho) {
            $anhoArray[] = JHTML::_('select.option', $anho->value, JText::_($anho->text));
        }
        $lists['resultado_anho'] = JHTML::_('select.genericlist', $anhoArray, 'nu_ano', 'class="inputbox" size="1"', 'value', 'text', $infmeta->nu_ano);
        ...
        $lists['resultado_mes'] = JHTML::_('select.genericlist', $mesArray, 'nu_mes', 'class="inputbox" size="1" onblur="validainfmetarepetido();" ', 'value', 'text', $infmeta->nu_mes);

        $this->assignRef('lists', $lists);
        $this->assignRef('infmeta', $infmeta);
